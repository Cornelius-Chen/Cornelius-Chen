...
The public repository also has an [independent composition replay](https://github.com/Cornelius-Chen/Jervis/blob/main/docs/case-composition.md). A field-level wood → gravel change rebuilds sound, page and combined observation, while retaining motion and an independent note. It renders two real WAV files, checks the HTML playback in Chromium and records an old Worker result as `STALE` after handoff. Fixed model responses make this an offline runtime demonstration; it is not a claim that a model autonomously planned the change or that listeners preferred the new sound.
